...
{
	cJSON *node = kmalloc(sizeof(cJSON), GFP_KERNEL);
...
static char *print_string_ptr(const char *str)
{
	...
	while ((token = *ptr) && ++len) {
		tmp = "\"\\\b\f\n\r\t";
		if (strnchr(tmp, strlen(tmp), token))
...
static char *print_array(cJSON *item, int depth, int fmt)
{
	...
	entries = kmalloc_array(numentries, sizeof(char *), GFP_KERNEL);
	if (!entries)
		return 0;
...
static char *print_object(cJSON *item, int depth, int fmt)
{
	...
	if (!numentries) {
		out = kmalloc(fmt ? depth + 4 : 3, GFP_KERNEL);
		if (!out)
			return 0;
		ptr = out;
		*ptr++ = '{';
	...
	if (!entries)
		return 0;
	names = kmalloc_array(numentries, sizeof(char *), GFP_KERNEL);
	if (!names) {
		kfree(entries);
		return 0;
	}
