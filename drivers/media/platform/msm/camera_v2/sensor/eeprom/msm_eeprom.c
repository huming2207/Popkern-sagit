...
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	if (e_ctrl->userspace_probe == 0) {
		...
		if (e_ctrl->cal_data.mapdata[0x01] == module_info[j].module_num) {
		...
		for (j = 0; j < (sizeof(project_info) / sizeof(struct project_info_t)); j++) {
			if (e_ctrl->cal_data.mapdata[0x0D] == project_info[j].pro_num) {
		...
		for (j = 0; j < (sizeof(sensor_info) / sizeof(struct sensor_info_t)); j++) {
			if (e_ctrl->cal_data.mapdata[0x0C] == sensor_info[j].sensor_num) {
		...
		if (strcmp(eb_info->eeprom_name, "sagit_imx386_semco") == 0) {
			memcpy((void *)g_cal_fadj_data, (void *)(e_ctrl->cal_data.mapdata+4910), 43);
			for (j = 0; j < 43; j++)
