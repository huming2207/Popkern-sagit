...
static const char *fts_get_config(struct fts_ts_info *info)
{
	...
	for (i = 0; i < pdata->config_array_size; i++) {
		if ((info->lockdown_info[6] == pdata->config_array[i].tp_vendor) &&
				(info->lockdown_info[2] == pdata->config_array[i].tp_color))
			break;
	}
...
static const char *fts_get_limit(struct fts_ts_info *info)
{
	...
	for (i = 0; i < pdata->config_array_size; i++) {
		if ((info->lockdown_info[6] == pdata->config_array[i].tp_vendor) &&
				(info->lockdown_info[2] == pdata->config_array[i].tp_color) &&
				(info->lockdown_info[3] == pdata->config_array[i].tp_hw_version))
			break;
	}
...
static int fts_chip_autotune(struct fts_ts_info *info)
{
	...
	if (ret2 < OK)
		log_error("%s fts initialization failed 3 times\n", tag);
...
static int fts_interrupt_install(struct fts_ts_info *info)
{
	...
	if (request_irq(info->client->irq, fts_interrupt_handler, IRQF_TRIGGER_LOW, info->client->name, info)) {
...
static int fts_fb_state_chg_callback(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	log_debug("%s %s: fts notifier begin!\n", tag, __func__);
	...
	if (evdata && evdata->data && val == FB_EVENT_BLANK && info) {
		...
		case FB_BLANK_POWERDOWN:
			...
			log_error("%s %s: FB_BLANK_POWERDOWN\n", tag, __func__);
		...
		case FB_BLANK_UNBLANK:
			...
			log_error("%s %s: FB_BLANK_UNBLANK\n", tag, __func__);
...
static ssize_t fts_selftest_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	snprintf(buf, sizeof(fts_info->result_type), "%d", fts_info->result_type);
	*pos += strlen(buf);
	return strlen(buf);
}
...
static ssize_t fts_selftest_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	if (!strncmp("short", buf, 5) || !strncmp("open", buf, 4)) {
		retval = fts_short_open_test();
	} else if (!strncmp("i2c", buf, 3))
		retval = fts_i2c_test();
...
static ssize_t fts_datadump_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	if (ret == 0)
		return ret;
	cnt1 = stm_fts_cmd_show(fts_info->dev, NULL, buf);
	if (cnt1 == 0)
		return cnt1;
	ret = stm_fts_cmd_store(fts_info->dev, NULL, "15", 2);
	if (ret == 0)
		return ret;
	cnt2 = stm_fts_cmd_show(fts_info->dev, NULL, buf + cnt1);
	if (cnt2 == 0)
		return cnt2;
	ret = stm_fts_cmd_store(fts_info->dev, NULL, "12", 2);
	if (ret == 0)
		return ret;
	cnt3 = stm_fts_cmd_show(fts_info->dev, NULL, buf + cnt1 + cnt2);
	if (cnt3 == 0)
		return cnt3;
	ret = fts_strength_frame_store(fts_info->dev, NULL, "1", 1);
	if (ret == 0)
		return ret;
	cnt4 = fts_strength_frame_show(fts_info->dev, NULL, buf + cnt1 + cnt2 + cnt3);
	if (cnt4 == 0)
		return cnt4;
	ret = fts_strength_frame_store(fts_info->dev, NULL, "2", 1);
	if (ret == 0)
		return ret;
	ret = fts_strength_frame_show(fts_info->dev, NULL, buf + cnt1 + cnt2 + cnt3 + cnt4);
	if (ret == 0)
		return ret;
	*pos += strlen(buf);
	return strlen(buf);
...
static int fts_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	info->test_cmd_dev = device_create(fts_cmd_class, NULL, DCHIP_ID_0, info, "fts_driver_test");
